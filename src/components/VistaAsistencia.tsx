...
const FilaPasoLista: React.FC<{
  integrante: Integrante;
  estado?: EstadoAsistencia;
  motivo?: string;
  justificacionPendiente?: Justificacion;
  finalizada: boolean;
  menuAbierto: boolean;
  puedePasar: boolean;
  puedeGestionarJustificaciones: boolean;
  onMenu: (id: string | null) => void;
  onMarcar: (id: string, estado: EstadoAsistencia) => void;
  onQuitar: (id: string) => void;
  onJustificar: (id: string, motivo: string) => void;
  onVerJustificacion: (id: string) => void;
}> = ({
  integrante,
  estado,
  motivo,
  ...
  const pulsacion = usePulsacionLarga(() => {
    if (!finalizada) onMenu(integrante.id);
  });
  return (
    <div
      ...
      className={`accion-mantener relative bg-white rounded-2xl p-3 border transition-all flex items-center justify-between gap-3 shadow-xs ${
        estado === 'Presente'
    ...
    >
      <div className="min-w-0">
        ...
        {estado === 'Justificado' && motivo && (
          <p className="text-[10px] text-amber-700 truncate mt-0.5">{motivo}</p>
        )}
        {justificacionPendiente && (
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        {!finalizada && puedePasar ? (
          <>
            <button
              onClick={() => onMarcar(integrante.id, 'Presente')}
              title="Presente"
            ...
            <button
              onClick={() => onMarcar(integrante.id, 'Ausente')}
              title="Ausente"
              className={`w-9 h-9 rounded-full flex items-center justify-center border transition-colors ${
                estado === 'Ausente'
            ...
            <button
              onClick={() => onMarcar(integrante.id, 'Justificado')}
              title="Justificado"
              className={`w-9 h-9 rounded-full flex items-center justify-center border transition-colors ${
                estado === 'Justificado'
            ...
            <button
              onClick={() => onMenu(menuAbierto ? null : integrante.id)}
              title="Opciones"
          ...
          <>
            ...
            {estado !== 'Presente' && estado !== 'Justificado' && puedeGestionarJustificaciones && (
              <button
                onClick={() => onJustificar(integrante.id, motivo || '')}
        ...
        {justificacionPendiente && puedeGestionarJustificaciones && (
          <button
            onClick={() => onVerJustificacion(justificacionPendiente.id)}
          ...
          {filtrados.map(integrante => {
            ...
            return (
              <FilaPasoLista
                ...
                justificacionPendiente={justificacionesPendientes.get(integrante.id)}
                finalizada={finalizada}
                menuAbierto={menuAbiertoId === integrante.id}
                puedePasar={puede('pasar_lista')}
                puedeGestionarJustificaciones={puede('gestionar_justificaciones')}
                onMenu={setMenuAbiertoId}
                onMarcar={handleMarcar}
                onQuitar={handleQuitar}
                onJustificar={(id, motivoFila) => {
                  setJustificandoId(id);
                  setMotivoTexto(motivoFila);
                }}
                onVerJustificacion={setJustificacionAbiertaId}
              />
            );
          })}
